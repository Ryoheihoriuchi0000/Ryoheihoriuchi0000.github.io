...
ウィジェットのタイルをタップすると、Apple 純正の **Shortcuts.app** を経由して対象アプリを起動します。

- 経由経路: SELECT ウィジェット → SELECT ホストアプリ (極短時間) → Shortcuts.app → 対象アプリ
- 送信内容: 起動対象アプリの **bundle ID** のみ (例: `jp.ne.paypay.app`)
- すべての処理は端末内で完結します
- Apple のサーバーや本アプリ開発者のサーバーへの送信はありません

事前に Shortcuts.app に **SELECT Widgets** という名前のショートカットを 1 回登録する必要があります。このショートカットは Apple から署名された安全な定義ファイルです。
